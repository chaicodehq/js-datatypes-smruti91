...
export function formatChaiMenu(items) {
   ...
   let filterItems = items.filter(item=> item.price !== 0 && item.price > 0 && typeof item.name === 'string' && item.name !== '');
      return filterItems.map(item=> `${item.name.toUpperCase()} - Rs.${item.price}`).join(" | ");
